...
def api_call(endpoint, method, expected_status, payload=None):
    ...
    for r in repo_list():
        if payload:
            response = requests.method(url, headers = headers, data=json.dumps(payload), verify=False)
        else:
            response = requests.method(url, headers = headers, verify=False)
    if response.status_code != expected_status:
        error_count += 1
    logging.info(f"{endpoint} returned {error_count} errors out of {num_repos} attempts. {float(error_count/num_repos)*100}% failure rate.")
    return error_count
...
def test_repo():
    assert api_call('user/repos', 'get', 201, "{'name':r}") == 0 
